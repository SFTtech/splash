...
class SplashViewer:
    def __init__(self):
        ...
        self.wpm = 100 # 70  # 300 w/min

    def run(self):
        gtk.main()

    def open_about(self, origin):
        d = self.builder.get_object("aboutdialog0")
        d.show()

    def close_about(self, a, b):
        d = self.builder.get_object("aboutdialog0")
        d.hide()

    def delete_window(self, *a):
        gtk.main_quit(a)

    def play(self, origin):
        if not self.splash:
    ...
    def canvas_draw(self, widget, cr):
        ...
        if len(txt) > 0:
            markup = "%s<span foreground='red'>%s</span>%s" % (txt[:center], txt[center], txt[center+1:])
            prefix = txt[:center]
        else:
            markup = ""
            prefix = ""
        ...
        # text metrics
        txtw, txth = (x/1024. for x in layout.get_size())
    ...
    def open_text(self, origin):
        ...
        chooser = gtk.FileChooserDialog(title="Open file to read...",
                                        ...
                                        buttons=(gtk.STOCK_CANCEL,
                                                 ...
                                                 gtk.STOCK_OPEN,
                                                 gtk.ResponseType.OK))
        for f in ("textfilefilter", "anyfilefilter"):
            filefilter = self.builder.get_object(f)
